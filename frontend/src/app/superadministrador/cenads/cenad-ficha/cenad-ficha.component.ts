...
export class CenadFichaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cenadService.getRecursosDeCenad(this.cenad.idCenad).subscribe((response =>
      this.recursos = this.cenadService.extraerRecursos(response)));
    this.cenadService.getSolicitudesDeCenad(this.cenad.idCenad).subscribe((response =>
