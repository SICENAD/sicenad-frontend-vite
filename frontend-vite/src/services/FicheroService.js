...
class FicheroService {
  ...
  async editarFichero(nombre, descripcion, archivo, nombreArchivoActual, idCenad, idRecurso, idCategoriaFichero, idFichero) {
  ...
  async fetchArchivoFichero(filename, idCenad, idRecurso) {
    const response = await this.utils.fetchArchivoConToken(`${this.utils.urlApi}/files/${idCenad}/docRecursos/${idRecurso}/${filename}`, 'GET', null)
